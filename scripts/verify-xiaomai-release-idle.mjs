...
const output=resolve('output/live2d-qa/xiaomai-release-idle');await mkdir(output,{recursive:true});
...
try{
  ...
  let previous=0;
  for(const seconds of [0,10,12,16,26]){
  ...
  await writeFile(resolve(output,'report.json'),JSON.stringify({idleSeconds:[0,10,12,16,26],states:['listening','thinking','speaking','affirming','idle'],errors},null,2));
  console.log(JSON.stringify({screenshots:11,errors}));
