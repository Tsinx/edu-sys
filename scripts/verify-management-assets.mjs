...
assert.equal(files.filter(f=>/\/mg-\d+\.webp$/.test(f.url)).length,build.imageCount);
...
const report={checkedAt:new Date().toISOString(),release,releaseId:manifest.releaseId,generatedImages:build.imageCount,originalReferenceAssets:files.length-build.imageCount,managementFiles:files.length,bytes:files.reduce((s,f)=>s+f.bytes,0),allHashesMatch:true,privateSourceFilesAbsent:true,originalNotesAbsent:true,cacheGroup:'management'};
await fs.writeFile('output/management-principles/qa-phase2/asset-integrity.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
