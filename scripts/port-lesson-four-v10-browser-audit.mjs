...
const base=process.env.PORT_L4_URL??'http://127.0.0.1:5173',out=process.env.PORT_L4_QA_OUT??'output/port-lesson-four-v10-qa';
...
try{
 ...
 for(const theme of ['light','dark']) for(const width of smoke?[1600]:[1600,390]) for(const mode of smoke?['teacher']:['teacher','projection']){
  await p.emulateMedia({colorScheme:theme});await p.setViewportSize({width,height:width===1600?1100:844});
  for(const n of smoke?[1,3,4,7,10,11,15,18,21,22,27,28,34,41]:Array.from({length:44},(_,i)=>i+1)){
   ...
   const result=await p.evaluate(()=>{
    ...
    const expected=c.classList.contains('l4-paper')?'rgb(33, 61, 72)':'rgb(234, 236, 231)';const heading=getComputedStyle(c.querySelector('h1'));const palette={expected,color:heading.color,fill:heading.webkitTextFillColor,background:getComputedStyle(c).backgroundColor};
    return {palette,title:c.getAttribute('aria-label'),bad,attrs,ratio:r.width/r.height,overflow:document.documentElement.scrollWidth>innerWidth+1,images:[...c.querySelectorAll('img')].length,scale};
   });
   assert.equal(result.palette.color,result.palette.expected);assert.equal(result.palette.fill,result.palette.expected);checks.push({theme,width,mode,page:n,...result});await p.locator('.port-l4-slide').screenshot({path:`${out}/${theme}-${mode}-${width}-${String(n).padStart(2,'0')}.png`});
  }
  console.log(`${theme} ${mode} ${width}px completed`);
 }
