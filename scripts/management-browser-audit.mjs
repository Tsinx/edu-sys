...
const phaseSlides=slides.filter(s=>s.lessonNumber>=5);
const selected=requested??(quick?phaseSlides.filter(s=>s.demo||s.table||s.diagram||s.layout==='cover').map(s=>s.index):phaseSlides.map(s=>s.index));
const out='output/management-principles/qa-phase2/web';await fs.mkdir(out,{recursive:true});
...
for(const width of quick||requested?[1600]:[1600,390]){
 ...
 for(const index of selected){
  await page.goto(`http://127.0.0.1:5173/management-preview.html?page=${index}&student`);
...
const report={scope:quick?'calibration':requested?'targeted':'complete',checked:results.length,expected:phaseSlides.length*2,results,errors,failures};
