...
await writeFile(join(destination,"release.json"),JSON.stringify({createdAt:new Date().toISOString(),sourceRevision:revision.status===0?revision.stdout.trim():null,web:JSON.parse(await readFile(join(destination,"web/offline-manifest.json"),"utf8")).releaseId,requiresGpu:false,requiresSubmodules:false},null,2));
