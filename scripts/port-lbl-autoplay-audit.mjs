...
try {
  ...
  // Leaving a page cancels its timer; the next page gets a full second.
  await select(14);await page.clock.runFor(600);await select(15);
  await page.clock.runFor(401);assert.equal(await progress(),'0');
  await page.clock.runFor(599);await button('暂停').waitFor();
  ...
  await select(58);await page.clock.runFor(1000);await button('暂停').waitFor();
  ...
  const report={initialFrame:true,delayMs:1000,bothLessons:true,pageEntryReset:true,timerCleanup:true,manualOverride:true,projectionKeyboardOverride:true,stopsAtEnd:true,noAutoAdvance:true,errors};
