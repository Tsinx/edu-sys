...
function buildSystemPrompt(session: StudySession, settings: AssistantPromptSettings): string {
  ...
  const tools = [
    ...
    `当前页面：第${position.localIndex}/${position.localTotal}页`,
...
export class StudyAssistantOrchestrator {
  ...
  startTurn(
    ...
    const systemPrompt = buildSystemPrompt(session, this.getPromptSettings());
    if (this.historyContext.get(session.id) !== systemPrompt) this.history.delete(session.id);
    this.historyContext.set(session.id, systemPrompt);
  ...
  private async *consumeProvider(
    ...
    try {
      ...
      const turns = [...(this.history.get(sessionId) ?? []), { user: userText, assistant: envelope.dialogue }];
