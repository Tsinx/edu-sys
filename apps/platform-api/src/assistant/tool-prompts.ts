...
export function buildClassroomToolPrompt(snapshot: { courseId: string; slide: { index: number; slideId: string; total: number } }, protocol: "legacy" | "realtime" = "legacy"): string {
  ...
  return [
    (legacy ? "你必须只返回一个 JSON 对象，禁止 Markdown、代码围栏、前后缀或额外说明。" : ""),
    ...(legacy ? responseInstructions : []),
    (legacy ? 'dialogue 必须是可直接朗读给课堂听众的简洁中文字符串；不要在 dialogue 中朗读 JSON、动作名或控制参数。' : ""),
    ...
    `教师在正式封面说“助教，开始第一讲”时，${legacy ? "静默" : ""}跳转到英法下注页${openingStartSlide ? `（内部全局第${openingStartSlide.index}页）` : ""}，不得直接播放地球仪。`,
    ...
    legacy ? "不需要控制课堂时actions返回空数组。操作不需要语音确认；需要教学回答时直接回答，不声称动作已经执行。"
      : "不需要控制课堂时直接回答，不调用工具。操作可以简短语音确认，收到执行结果前不得声称已经完成；操作后讲解依据更新后的页面。",
    `可跳转的已建设课次：${readyLessonMap}。其余未在已建设目录中的讲次不得虚构标题、页码或教学内容。`,
    (legacy ? 'schema 固定为 "edu.classroom.assistant.response"，version 固定为 "1.0"。' : ""),
    (legacy ? '回答示例：{"replyKind":"answer","dialogue":"港口通常由水域、陆域和连接设施构成。","actions":[],"schema":"edu.classroom.assistant.response","version":"1.0"}' : ""),
    (legacy ? '操作示例：{"replyKind":"control","dialogue":"","actions":[{"type":"slides.next"}],"schema":"edu.classroom.assistant.response","version":"1.0"}' : ""),
  ].join("\n");
}
