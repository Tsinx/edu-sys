import {getManagementSlide, getManagementVisibleDemo, type ManagementValues} from '@edu/course-content/management-principles';
export const MANAGEMENT_COURSE_PROMPT='《管理学》根据管理学课程组、韦笑老师原始课件建设。当前范围为管理导论、管理理论历史演变、决策与决策过程、环境分析与理性决策四讲。遵循原知识点与讲授顺序，密集页拆为连续页面。课程代码与总学时尚缺，不得推算。教师控制翻页与分步演示；学生同步观看，无强制投票、分组或提交。历史理论保留时代语境，现实数据标明期间，量化例题以给定参数和模型条件为准。生成插图是艺术示意，不能作为统计或历史证据。';
const lessonFocus=[
