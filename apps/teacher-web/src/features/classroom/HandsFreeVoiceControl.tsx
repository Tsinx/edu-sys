...
import { voiceStartupLabels, type VoiceStartupStage } from "./voice-startup";
...
export function HandsFreeVoiceControl({ mode = "handsfree", disabled, assistantBusy, realtime, unavailableReason }: HandsFreeVoiceControlProps) {
  ...
  const [startupStage, setStartupStage] = useState<VoiceStartupStage>();
  ...
  useEffect(() => {
    ...
    setCollecting(false);
    setEndingPending(false);
    if (!enabled || blocked || !realtime) return;
    ...
    const begin = async () => {
      try {
        const capture = await startVoiceCapture({
          ...
          onStartupStage: stage => { if (!controller.signal.aborted) setStartupStage(stage); },
          realtime: {
            prepare: () => realtime.prepare(), begin: () => realtime.begin(), append: samples => realtime.append(samples), cancel: () => realtime.cancel(),
            commit: async () => {
  ...
  }, [enabled, blocked, cycle, mode, keywordModel, realtime]);
