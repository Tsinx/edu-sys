...
export function PortLessonFourTeacherGuide({page,onPage,onOpen}:{page:PortLessonFourPage;onPage:(page:number)=>void;onOpen:(cueId:PortDemoCueId)=>void}) {
  ...
  return <aside className="port-l4-teacher-guide" aria-label="第4讲教师授课提示">
    ...
    <button onClick={()=>onOpen(lab.cueId)}>切入{lab.name}标准演示</button>
    ...
    <details><summary>四段演示与任务导航</summary>{PORT_LESSON_FOUR_LABS.map(l=><div key={l.unit}><h3>{l.name}</h3><button onClick={()=>onOpen(l.cueId)}>切入标准演示</button><button className="port-l4-jump" onClick={()=>onPage(l.demoPage)}>第{l.demoPage}页 · 观察问题</button>{l.taskPage&&<button className="port-l4-jump" onClick={()=>onPage(l.taskPage)}>第{l.taskPage}页 · 学生实机</button>}</div>)}</details>
