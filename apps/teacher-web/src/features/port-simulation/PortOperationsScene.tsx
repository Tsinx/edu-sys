...
export const PortOperationsScene = forwardRef<PortSceneHandle, {
    ...
    const construction = JSON.stringify({ ...props.view.plan.equipment, dispatch: undefined });
    ...
    useImperativeHandle(ref, () => ({ focus, camera(name) { const r = runtime.current; if (!r)
            return; following.current = false; const p = name === "yard" ? [135, 160, -10] : name === "sea" ? [160, 230, -330] : [198, 240, -290]; r.camera.position.set(p[0]!, p[1]!, p[2]!); r.controls.target.set(0, 0, name === "yard" ? 30 : name === "sea" ? -165 : -87); } }), []);
    useEffect(() => {
        ...
        scene.fog = new THREE.Fog("#c6dfe2", 640, 1500);
        const camera = new THREE.PerspectiveCamera(43, 1, .5, 2400);
        ...
        for (let i = 0; i < 4; i++)
            marker(`anchor:${i}`, anchorPosition(i), `候泊锚位 ${i + 1}`, 25);
        for (let i = 0; i < 2; i++)
            marker(`berth:${i}`, berthPosition(i), `泊位 ${i ? "B · 大型船适配" : "A"}`, 17);
        const buoys = new THREE.Group();
        for (const x of [-116, 116])
            for (let z = -120; z >= -290; z -= 28) {
                const buoy = new THREE.Mesh(new THREE.CylinderGeometry(.6, 1, 3, 6), new THREE.MeshStandardMaterial({ color: x < 0 ? 0xcc7356 : 0x5fc8af }));
        ...
        observer.observe(h);
        resize();
        runtime.current = { camera, controls, positions };
        let lastView: PortView | null = null, anchorTime = performance.now(), frame = 0, shadow = -Infinity, lastInset = -1;
        const vesselPoint = (id: string, clock: number) => { const v = latest.current.view.vessels.find(v => v.id === id)!; const c = v.call; if (c.move) {
        ...
        const point = (location: string, index = 0, clock = 0) => { const [kind, id] = location.split(":"); if (kind === "ship")
            return vesselPoint(id!, clock).add(new THREE.Vector3(-24 + index % 8 * 8, 6 + Math.floor(index / 32) * 2.8, -5.1 + Math.floor(index % 32 / 8) * 3.4)); if (["yard", "handoff", "pickup"].includes(kind!)) {
        ...
        renderer.setAnimationLoop(now => {
            if (document.hidden || now - frame < 33)
                return;
            frame = now;
            if (now - shadow > 750) {
        ...
        setStatus("ready");
        return () => { renderer.setAnimationLoop(null); observer.disconnect(); controls.dispose(); renderer.domElement.removeEventListener("pointerdown", pointerDown); renderer.domElement.removeEventListener("pointerup", pointerUp); renderer.domElement.removeEventListener("contextmenu", context); renderer.domElement.removeEventListener("dragover", dragOver); renderer.domElement.removeEventListener("drop", drop); renderer.domElement.removeEventListener("keydown", key); renderer.domElement.removeEventListener("webglcontextlost", lost); world.dispose(); env.dispose(); sun.shadow.map?.dispose(); renderer.dispose(); renderer.forceContextLoss(); renderer.domElement.remove(); runtime.current = null; };
    }, [construction, retry]);
