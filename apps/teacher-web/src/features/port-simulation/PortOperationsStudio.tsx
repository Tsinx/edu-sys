...
import { PortSubmissionPanel } from "./PortSubmissionPanel";
import type { SubmissionContext } from "./submission-api";
...
export interface PortOperationsProps {
    ...
    demonstrationStorage?: LabStorage | null;
    onDemonstrationFrame?: (view:PortView,speed:number) => void;
    onDemonstrationSnapshot?: (summary:string) => void;
    onDemonstrationError?: (message:string) => void;
...
export function PortOperationsStudio({ submission, navigationUnit, onNavigateModule, onNavigationError, storage, storageScope, sourceLabel = "独立实验", initialTrainingMode = "practice", trainingModeLocked = false, initialMode = "flow", initialScenario = "regular", onLegacy, courseUnit: practiceUnit, courseSelection = "full", demonstration = false, embeddedStage = false, demonstrationStorage, onDemonstrationFrame, onDemonstrationSnapshot, onDemonstrationError, learningStageLocked = false, courseProgress = [], courseSaveError, onSelectCourse, onCourseComplete, offerTutorial = true }: PortOperationsProps) {
    ...
    useEffect(()=>{
      ...
      teachingCallbacks.current.onDemonstrationFrame?.({...v,status:!r.demoPlaying&&v.status==="running"?"paused":v.status},r.speed);
      teachingCallbacks.current.onDemonstrationSnapshot?.(JSON.stringify({
        unit:practiceUnit,second:v.second,status:v.status,playing:r.demoPlaying,
        checkpoint:r.demonstration?.title??"已载入，等待教师播放",goals:r.lesson?.goals,
        vessels:v.vessels.map(s=>({id:s.id,stage:s.call.stage,waitReason:s.waitReason})),
        unloaded:v.boxes.filter(b=>b.unloadedAt!==null).length,
        loaded:v.boxes.filter(b=>b.loadedAt!==null).length,
        delivered:v.boxes.filter(b=>b.deliveredAt!==null).length,
        berths:v.berths,channel:v.channel,resources:v.resources,
        batches:v.batches.map(b=>({id:b.id,flow:b.flow,summary:b.summary})),
        recentNotices:v.notices.slice(-3)
      }));
    },[demonstration,practiceUnit,v,r.demoPlaying,r.demonstration,r.lesson]);
    useEffect(()=>{if(demonstration&&r.error)teachingCallbacks.current.onDemonstrationError?.(r.error);},[demonstration,r.error]);
    ...
    const [stageFullscreen, setStageFullscreen] = useState(false), [stagePanel, setStagePanel] = useState<"operations" | "plans" | "goals" | null>(embeddedStage ? null : "operations");
    const stageLayout=stageFullscreen||embeddedStage;
    const scene = useRef<PortSceneHandle>(null), file = useRef<HTMLInputElement>(null), root = useRef<HTMLElement>(null);
    const returnUi = useRef({ tab, selected, batchId, stagePanel });
    const enterTutorial = async (unit: PortCourseUnit) => {
        if (practice.busy || practice.view?.mode === "battle" && ["running", "paused"].includes(practice.view.status)) return;
        if (practice.view?.status === "running") await practice.send({ kind: "pause" });
        returnUi.current = { tab, selected, batchId, stagePanel };
        setOffer(false); setTutorialUnit(unit); setTab(portCourseDefinition(unit).tab); setSelected("S01"); setBatchId("S01-I1"); setStagePanel("operations");
    };
    const exitTutorial = () => {
        setTutorialUnit(null); setOffer(false);
        setTab(returnUi.current.tab); setSelected(returnUi.current.selected); setBatchId(returnUi.current.batchId); setStagePanel(returnUi.current.stagePanel);
    };
    const retry = async (newSeed = false, entry?: Parameters<typeof r.retry>[1]) => { await r.retry(newSeed, entry); if (!tutorial && !demonstration && r.view?.mode === "practice") setOffer(true); };
    ...
    const locked = demonstration || r.sealed || r.sealing || !v || !["running", "paused"].includes(v.status);
    ...
    const navigationLocked = tutorial || !demonstration && v?.mode === "battle" && ["running", "paused"].includes(v.status);
    const send = async (command: PortCommand) => { if (demonstration) return; if (command.kind === "dispatch" && v?.status === "ready" && (!courseUnit || courseUnit === "planning")) {
    ...
    useEffect(() => { if (r.lesson?.complete && !demonstration && !tutorial && courseUnit) onCourseComplete?.(courseUnit); }, [r.lesson?.complete, courseUnit, demonstration, tutorial]);
    const revealTutorialStep = () => {
        const step = r.tutorial?.current;
        if (!step) return;
        setTab(availableTabs.includes(step.tab) ? step.tab : definition?.tab ?? "ships");
        setStagePanel(step.id === "dossier" || step.id === "arrival" ? "plans" : "operations");
        setSelected(step.focus);
        const box = v?.boxes.find(b => b.id === step.focus);
        if (box) { setBatchId(box.batchId); if (step.id !== "issue-open") void r.inspect(box.id); }
        else if (v?.batches.some(b => b.id === step.focus)) setBatchId(step.focus);
        if(step.sceneGesture?.destinations.length) scene.current?.fit([step.sceneGesture.source,step.sceneGesture.destinations[0]!]);
        else scene.current?.focus(step.focus);
    };
    useEffect(() => { revealTutorialStep(); }, [r.tutorial?.current?.id, !!r.tutorial?.current?.sceneGesture]);
    useEffect(() => {
    ...
    if (!v)
        return <section ref={root} className="port-ops port-loading" data-stage-fullscreen={stageLayout} data-embedded-stage={embeddedStage}><Ship size={36}/><h2>{definition?.title ?? "48 小时港口综合实训"}</h2><p>{r.error || "正在载入本机场次与业务内核…"}</p>{r.error && <button onClick={() => void retry()}>保留原记录并开始新挑战</button>}</section>;
    return <section className="port-ops" ref={root} data-experience={tutorial ? "tutorial" : demonstration ? "demonstration" : "practice"} data-status={v.status} data-mode={v.mode} data-second={v.second} data-course={courseUnit ?? "full"} data-demo={demonstration} data-stage-fullscreen={stageLayout} data-embedded-stage={embeddedStage} data-stage-panel={stagePanel ?? "none"}>
  ...
  {demonstration && <section className="port-demo-banner" aria-label="课程标准演示"><div><span className="port-demo-badge"><CirclePlay size={18}/>课程标准演示</span><strong>{r.demonstration?.title ?? `观看${definition?.title}的完整操作`}</strong><p>{r.demonstration?.explanation ?? "自动执行真实业务指令并展示现场结果。可播放、暂停或单步观看；演示不写入自主练习进度。"}</p></div><div className="port-demo-controls"><button className="primary" disabled={r.busy || r.lesson?.complete} onClick={() => r.setDemoPlaying(!r.demoPlaying)}>{r.demoPlaying ? <Pause size={16}/> : <Play size={16}/>} {r.demoPlaying ? "暂停演示" : "播放演示"}</button><button disabled={r.busy || r.lesson?.complete} onClick={() => void r.demoStep()}><StepForward size={16}/>下一步演示</button><button onClick={() => void retry()}>重播本段</button>{onSelectCourse && <button onClick={() => void chooseCourse(courseSelection)}>返回自主练习</button>}</div></section>}
  {courseSaveError && <p role="status">{courseSaveError}</p>}
  <header className="port-header"><div className="port-brand"><span className="port-brand-icon"><Anchor size={25}/></span><div><span className="port-eyebrow">PORT OPERATIONS · {sourceLabel}</span><h1>{definition?.title ?? "48 小时港口综合实训"}</h1></div></div><div className="port-clock"><span className={`port-live ${v.status === "running" ? "on" : ""}`}/><strong>{portTime(v.second)}</strong><span>{statusNames[v.status]} · {r.lesson ? `本段 ${hours(r.lesson.elapsed)}` : `第 ${Math.min(6, Math.floor(v.second / 28800) + 1)} 班`}</span></div><div className="port-controls"><span className="port-mode">{tutorial ? "操作教学" : demonstration ? "观察模式" : v.mode === "practice" ? "教学模式" : "实战模式"}</span>{!demonstration && (v.status === "ready" ? <button data-tutorial-target="clock:start" disabled={r.sealed || r.sealing} className="primary" onClick={() => { setShowRules(false); void send({ kind: "start" }); }}><Play size={15}/>{courseUnit ? "开始本段" : "开始值班"}</button> : v.mode === "practice" && ["running", "paused"].includes(v.status) ? <button data-tutorial-target={v.status === "running" ? "clock:pause" : "clock:resume"} disabled={r.sealed || r.sealing} className="primary" onClick={() => void send({ kind: v.status === "running" ? "pause" : "resume" })}>{v.status === "running" ? <Pause size={15}/> : <Play size={15}/>} {v.status === "running" ? "暂停" : "继续运行"}</button> : <span className="port-speed">{v.mode === "battle" ? "60× 固定" : "本段已结束"}</span>)}{v.mode === "practice" && <select aria-label="运行速度" value={r.speed} onChange={e => r.setSpeed(Number(e.target.value))}>{tutorial&&<option value={10}>10× · 航行观察</option>}<option value={30}>30×</option><option value={60}>60×</option><option value={120}>120×</option><option value={300}>300×</option><option value={600}>600×</option></select>}{!demonstration && !tutorial && <button className="port-tutorial-entry" disabled={navigationLocked || r.busy} onClick={() => { setManualOffer(true); setOffer(true); }}><BookOpen size={17}/>操作教学</button>}{onSelectCourse && !demonstration && !tutorial && <button className="port-demo-entry" disabled={navigationLocked || r.busy} title={navigationLocked ? "实战结束后可观看演示" : "观看内置课程示范，不影响当前练习记录"} onClick={() => void chooseCourse(courseSelection, true)}><CirclePlay size={17}/>标准演示</button>}<button className="port-fullscreen-entry" aria-label={stageFullscreen ? "退出舞台全屏" : "舞台全屏"} onClick={() => void toggleFullscreen()}>{stageFullscreen ? <Minimize size={17}/> : <Maximize size={17}/>} {stageFullscreen ? "退出全屏" : "舞台全屏"}</button><button aria-label="查看规则" onClick={() => setShowRules(!showRules)}>规则</button></div></header>
  ...
  </div></main><aside className="port-workbench"><nav className="port-tabs" aria-label="业务工作台">{[["ships", "船舶", Ship], ["cargo", "货批", Boxes], ["resources", "调度", Users], ["plan", "规划", Map], ["review", "复盘", Flag]].filter(([id]) => availableTabs.includes(String(id))).map(([id, label, Icon]) => { const I = Icon as typeof Ship; return <button data-tutorial-target={`tab:${id}`} key={String(id)} aria-selected={tab === id} onClick={() => { setTab(String(id)); if (tutorial) void r.observe(`${id === "resources" ? "resources" : id === "review" ? "review" : "tab"}-open`); }}><I size={16}/>{String(label)}</button>; })}</nav>
   {tutorial && r.tutorial?.current?.gesture === "drag" && <div className="port-tutorial-rack" data-tutorial-rack="true"><p>选择对象 → 拖入目标，或直接点目标完成同一操作</p>{(() => {
      const step = r.tutorial!.current!, source: PortDrag = step.focus.includes("-") ? { kind: "batch", id: step.focus } : { kind: "ship", id: step.focus };
      return <><button className="port-tutorial-source" data-tutorial-target={source.kind === "ship" ? "ship-drag" : `batch:${source.id}`} {...dragProps(source)} onClick={() => { if (source.kind === "batch") setBatchId(source.id); else setSelected(source.id); }}>{source.id} · {source.kind === "ship" ? "船舶" : "货批"}</button><div className="port-tutorial-destinations">{step.targets.filter(t => t.startsWith("destination:") || t.startsWith("yard-target:")).map(t => { const target = t.startsWith("destination:") ? t.slice(12) : t.slice(12); return <button key={t} data-tutorial-target={t} {...dropProps(target, drop)} onClick={() => drop(source, target)}>{target.startsWith("berth:") ? `泊位 ${target.endsWith("0") ? "A" : "B"}` : target.startsWith("anchor:") ? `候泊 ${Number(target.slice(7)) + 1}` : v.yards.find(y => y.id === target)?.name ?? target}</button>; })}</div></>;
   })()}</div>}
   <div className="port-panel">
    {tab === "ships" && currentShip && <><div className="port-panel-heading"><span className="port-eyebrow">VESSEL DOSSIER</span><h2>{currentShip.name}<button aria-label="定位船舶" onClick={() => scene.current?.focus(currentShip.id)}><Focus size={16}/></button></h2></div><select aria-label="当前船舶" value={currentShip.id} onChange={e => setSelected(e.target.value)}>{v.vessels.map(s => <option key={s.id} value={s.id}>{s.id} · {stageNames[s.call.stage]}</option>)}</select><p data-tutorial-target="ship-status" className="port-advice">{currentShip.waitReason}</p><PortNavigationReadout ship={currentShip} view={v} speed={r.speed}/><dl data-tutorial-target="ship-reference" className="port-facts"><div><dt>航次</dt><dd>{currentShip.voyage}</dd></div><div><dt>航前船员名册</dt><dd>{currentShip.crew} 人</dd></div><div><dt>实际到港</dt><dd>{currentShip.actualArrival === null ? "尚未到达" : portTime(currentShip.actualArrival)}</dd></div><div><dt>抵达泊位</dt><dd>{currentShip.call.berthArrivedAt === null ? "尚未抵达" : portTime(currentShip.call.berthArrivedAt)}</dd></div><div><dt>实际靠妥</dt><dd>{currentShip.call.milestones.secure === undefined ? "尚未靠妥" : portTime(currentShip.call.milestones.secure)}</dd></div><div><dt>周转承诺{currentShip.actualArrival === null ? "（预测）" : ""}</dt><dd>{portTime(currentShip.deadline)}</dd></div></dl>
     <div className="port-operation"><h3>{courseUnit === "departure" ? "离港安排" : courseUnit === "cargo" ? "船岸作业" : "泊位计划与通行"}</h3>{(!courseUnit || courseUnit === "arrival") && <><label>目的位置<select aria-label="目的位置" value={destination} onChange={e => setDestination(e.target.value)}><option value="berth:0">泊位 A · 普通船</option><option value="berth:1">泊位 B · 普通 / 大型船</option>{v.anchors.map((_, i) => <option key={i} value={`anchor:${i}`}>候泊锚位 {i + 1}</option>)}</select></label><div className="port-inline"><label>计划时刻（开局后分钟）<input type="number" min={Math.ceil(v.second / 60)} max={3240} value={plannedMinute} onChange={e => setPlannedMinute(Number(e.target.value))}/></label><button disabled={demonstration || (locked && v.status !== "ready") || !destination.startsWith("berth:")} onClick={() => send({ kind: "plan-berth", callId: currentShip.id, berth: Number(destination.split(":")[1]), at: plannedMinute * 60 })}>预约</button></div></>}<div className="port-actions">{(!courseUnit || courseUnit === "arrival") && <button data-tutorial-target="ship-move" disabled={locked} className="primary" onClick={() => send({ kind: "move", callId: currentShip.id, target: destination.startsWith("berth:") ? "berth" : "anchor", slot: Number(destination.split(":")[1]) })}>申请进港 / 移泊</button>}{(!courseUnit || courseUnit === "cargo") && <button data-tutorial-target="ship-work" disabled={locked} onClick={() => send({ kind: "work", callId: currentShip.id, running: !currentShip.call.working })}>{currentShip.call.working ? "停止接续装卸" : "组织装卸"}</button>}{(!courseUnit || courseUnit === "departure") && <button data-tutorial-target="ship-depart" disabled={locked} onClick={() => send({ kind: "depart", callId: currentShip.id })}>离泊出港</button>}</div><small>{courseUnit === "departure" ? "确认装卸结果与出口岸准备，船舶实际驶离后释放资源。" : "预约不占用实际泊位。申请执行时检查手续、航道和目的位置。"}</small></div>
     <details open className="port-details"><summary>适用船舶手续与回执</summary><p>航次申请核对航次号，检疫及边检核对航前船员名册人数。基础资料已预填；核对后发起委托，机构按情境规则返回结果。三项入港申报可连续提交，货批核验也可并行；本实验将入港回执合并检查，不是完整的法定口岸流程。离港资料预核对可提前完成，不等于取得出口岸许可证；实际离港仍检查装卸、资料和通行条件。</p>{(["entry", "health", "border", "departure"] as const).filter(kind => !courseUnit || (courseUnit === "departure" ? kind === "departure" : kind !== "departure")).map(kind => <DocForm key={`${currentShip.id}-${kind}`} id={`doc-${currentShip.id}-${kind}`} title={PORT_DOC_NAMES[kind]} document={currentShip.call.docs[kind]} disabled={locked} submit={value => send({ kind: "document", callId: currentShip.id, document: kind, value })}/>)}</details></>}
    {tab === "cargo" && batch && <><div className="port-panel-heading"><span className="port-eyebrow">CARGO OPERATIONS</span><h2>货批与逐箱交接</h2></div><div className="port-batch-list">{v.batches.filter(b => b.summary.loaded + b.summary.delivered < b.summary.total).map(b => <button key={b.id} data-tutorial-target={`batch:${b.id}`} data-batch={b.id} className={batch.id === b.id ? "selected" : ""} {...dragProps({ kind: "batch", id: b.id })} onClick={() => setBatchId(b.id)} onContextMenu={e => { e.preventDefault(); setBatchId(b.id); }}><span>{b.id}</span><small>{b.flow === "import" ? "卸" : "装"} {b.summary.total} 箱</small></button>)}</div><h3>{batch.name}</h3><p className="port-advice">承诺{v.vessels.find(s => s.id === batch.callId)?.actualArrival === null ? "（预测）" : ""}：{portTime(batch.deadline)}。{batch.summary.issues ? `${batch.summary.issues} 箱需要核查；其余箱正常接续。` : "批次分配一次，实际交接逐箱记账。"}</p><div className="port-inline"><label>目标堆场<select data-tutorial-target="batch-destination" aria-label="货批目标堆场" value={batch.targetYard ?? ""} disabled={demonstration || locked && (v.status !== "ready" || !!courseUnit && courseUnit !== "planning")} onChange={e => send({ kind: "assign-yard", batchId: batch.id, yardId: e.target.value })}><option value="" disabled>选择目标</option>{v.yards.map(y => <option key={y.id} value={y.id}>{y.name} · {useNames[y.use]} · 余 {y.available}</option>)}</select></label><button onClick={() => batch.targetYard && scene.current?.focus(batch.targetYard)}><Focus size={16}/></button></div><p data-tutorial-target="cargo-result">已卸 {batch.summary.unloaded} · 已装 {batch.summary.loaded} · 已提离 {batch.summary.delivered}</p><dl data-tutorial-target="batch-reference" className="port-facts"><div><dt>提单原始关联</dt><dd>{batch.reference}</dd></div><div><dt>承运交付许可</dt><dd>{batch.carrier ? "航前记录已接收" : "待核对"}</dd></div>{batch.flow === "export" && <div><dt>VGM 记录</dt><dd>{batch.vgm ? "已接收" : "待核验"}</dd></div>}</dl><DocForm id={`doc-${batch.id}`} title="货物资料核验" document={batch.document} disabled={locked} submit={value => send({ kind: "batch-document", batchId: batch.id, value })}/>
     <details open className="port-details"><summary>箱记录 · {batch.summary.total} 箱</summary><div data-tutorial-target="box-list" className="port-box-list">{v.boxes.filter(b => b.batchId === batch.id).map(b => <button data-tutorial-target={`box:${b.id}`} key={b.id} onClick={() => { void r.inspect(b.id); setSelected(b.id); scene.current?.focus(b.id); }} className={b.issue === "open" || b.issue === "reported" ? "has-issue" : ""}><span>{b.id}</span><small>{portLocationName(b.location)} · {b.recordCount} 次记录</small>{["open", "reported"].includes(b.issue) && <span>待核查</span>}</button>)}</div></details>
     {r.box && r.box.batchId === batch.id && <section data-tutorial-target="box-record" className="port-box-detail"><h3>{r.box.id} · 交接账本</h3><p>{portLocationName(r.box.location)}</p>{["open", "reported"].includes(r.box.issue) && <div><p>封志与交接资料不一致。将该箱送入待核查区，委托复核。</p><select data-tutorial-target="inspection" aria-label="核查目标" value={inspectYard} onChange={e => setInspectYard(e.target.value)}>{v.yards.filter(y => y.use === "inspection").map(y => <option key={y.id} value={y.id}>{y.name}</option>)}</select><button data-tutorial-target="inspect-submit" disabled={locked || r.box.issue === "reported"} onClick={() => { send({ kind: "inspect", boxId: r.box!.id, yardId: inspectYard }); void r.inspect(r.box!.id); }}>送检并委托核查</button></div>}<ol>{r.box.history.map((h, i) => <li key={i}><time>{portTime(h.at)}</time><strong>{h.action}</strong><span>{portLocationName(h.from)} → {portLocationName(h.to)}</span><small>{h.resource}</small></li>)}</ol><button onClick={() => void r.inspect(r.box!.id)}>刷新此箱记录</button></section>}</>}
    ...
    {tab === "review" && !courseUnit && <><div className="port-panel-heading"><span className="port-eyebrow">REVIEW & HANDOVER</span><h2>复盘与交班</h2></div><div className="port-score-grid">{[["货物履约", v.score.cargo, 50], ["船舶流程", v.score.process, 20], ["经营效率", v.score.efficiency, 20], ["交班完整", v.score.handover, 10]].map(([label, n, max]) => <div key={String(label)}><small>{label}</small><strong>{fmt(Number(n), 2)} <em>/ {max}</em></strong></div>)}</div><p>流程完成 {fmt(v.score.processCompletion, 2)} 分 · 按时离港 {fmt(v.score.processTimeliness, 2)} 分（{v.score.onTimeDepartures}/{v.score.dueShips} 艘到期船舶，评分版本 {v.score.scoringVersion}）</p><p>扣分 {v.score.deductions} · 到期 {v.score.dueBoxes} 箱 · 按时 {v.score.onTime} 箱 · 逾期未按时 {v.score.overdue} 箱</p><p>{v.status === "interrupted" ? "本场已中断，只用于诊断，不能选为最终成绩。" : v.status === "completed" ? "48 小时已结束。未到期任务可结转，逾期结果不会被交班消除。" : "当前为进度诊断，到期任务将逐步纳入评价。"}</p><p>单位成本 {fmt(v.score.unitCost, 3)} / 基准 {v.score.referenceUnitCost === null ? "计算中" : fmt(v.score.referenceUnitCost, 3)} 教学点。翻箱 {v.rehandles} 次，运输 {fmt(v.distance)} 距离单位。</p><details className="port-details"><summary>等待时间与原始运营指标</summary>{v.statistics && <p>设定平均间隔 {fmt(v.statistics.configuredMeanGap, 1)} 分钟 / 基准平均服务 {fmt(v.statistics.referenceMeanService, 1)} 分钟；本局首报平均间隔 {fmt(v.statistics.sampleMeanGap, 1)} 分钟 / 参考平均服务 {fmt(v.statistics.sampleMeanService, 1)} 分钟。单局样本允许波动。</p>}{v.vessels.map(s => <p key={s.id}>{s.id} · 泊位 {fmt(s.call.wait.berth / 60)} 分 · 锚地 {fmt(s.call.wait.anchor / 60)} 分 · 航道 {fmt(s.call.wait.channel / 60)} 分 · 资料 {fmt(s.call.wait.documents / 60)} 分 · 调度 {fmt(s.call.wait.dispatch / 60)} 分</p>)}</details>
     ...
     <div className="port-actions"><button disabled={!(["completed", "interrupted"].includes(v.status)) || !v.score.referenceReady} onClick={() => void r.exportReport()}><Download size={15}/>导出当前报告</button></div><div className="port-history"><h3>本次实验</h3>{r.history.entries.filter(e => e.id === r.history.active).map(e => <div key={e.id}><div className="port-row"><strong>{e.mode === "battle" ? "实战" : "教学"} · {e.seed} · {e.rules === "port-operations/3.0" ? "原通航规则" : "航行规则 1.0"}</strong><span>{statusNames[e.status]}</span></div><small>{e.created.slice(0, 16).replace("T", " ")} · {hours(e.second)} · {e.score === null ? "诊断记录" : `${e.score} 分`}</small><div className="port-actions"><button disabled={r.busy} onClick={() => void r.openHistory(e)}>打开</button><button onClick={() => void retry(false, e)}>同船期重练</button>{e.mode === "battle" && e.status === "completed" && <button disabled={e.score === null} onClick={() => void r.selectFinal(e.id)}>{r.history.final === e.id ? <><Check size={14}/>当前结果</> : "查看当前结果"}</button>}</div></div>)}</div></>}
   {tab === "review" && submission && !demonstration && !tutorial && <PortSubmissionPanel key={`${submission.actorId}:${courseUnit ?? "full"}`} context={submission} storage={storage} unit={courseUnit ?? "full"} runId={r.runId} eligible={!!courseUnit || v.status === "completed" && v.mode === "battle" && v.score.referenceReady} busy={r.sealing} sealed={r.sealed} seal={r.seal}/>}
   {tab === "review" && r.lesson && <><div className="port-panel-heading"><span className="port-eyebrow">LESSON REVIEW</span><h2>本段作业复核</h2></div><p>{r.lesson.complete ? "本段目标已由实际作业状态验证。" : "尚未完成的目标可在现场继续练习。"}{demonstration ? " 当前为课程标准演示，不计入自主练习记录。" : " 这里保留本段的指令、判定和逐箱结果。"}</p>{r.lesson.goals.map(g => <p key={g.id}>{g.done ? "✓" : "○"} {g.label}</p>)}<p>本段运行 {hours(r.lesson.elapsed)} · 实际交接 {v.boxes.reduce((n, b) => n + b.recordCount, 0)} 条</p><PortPerformanceSummary value={r.lesson.performance}/><details className="port-details" open><summary>本段处置记录</summary>{v.attempts.slice().reverse().map((a, i) => <div className="port-attempt" key={i}><strong>{portTime(a.at)} · {a.object}</strong><p>{a.message}</p><small>{a.outcome} · {a.rule}</small></div>)}</details>{!demonstration && !tutorial && <><button onClick={() => void r.exportReport()}><Download size={15}/>导出本段记录</button><h3>本段当前记录</h3>{r.history.entries.filter(e => e.id === r.history.active).map(e => <div key={e.id} className="port-course-history"><span>{e.created.slice(0, 16).replace("T", " ")} · {statusNames[e.status]}</span><button onClick={() => void r.openHistory(e)}>打开记录</button></div>)}</>}</>}
   </div><footer className="port-panel-footer"><button disabled={r.busy} onClick={() => void retry()}><RotateCcw size={14}/>{courseUnit ? demonstration ? "重播本段" : "重练本段" : "同船期重练"}</button>{!courseUnit && <button disabled={r.busy} onClick={() => void retry(true)}>新船期挑战</button>}<button disabled={demonstration || tutorial} onClick={() => file.current?.click()}><Upload size={14}/>导入</button><input ref={file} type="file" accept=".json,application/json" hidden onChange={e => { const f = e.target.files?.[0]; if (f)
        void f.text().then(r.load); e.target.value = ""; }}/></footer>
  </aside></div>
  {stageLayout && <nav className="port-stage-dock" aria-label="全屏舞台工具"><button aria-pressed={stagePanel === "operations"} onClick={() => setStagePanel(stagePanel === "operations" ? null : "operations")}><PanelRight size={16}/>操作面板</button><button aria-pressed={stagePanel === "plans"} onClick={() => setStagePanel(stagePanel === "plans" ? null : "plans")}><Map size={16}/>船期与计划</button>{courseUnit && <button aria-pressed={stagePanel === "goals"} onClick={() => setStagePanel(stagePanel === "goals" ? null : "goals")}><ListChecks size={16}/>课程目标</button>}<span>{v.channel ? `${v.channel} 通行中` : "航道可用"}</span></nav>}
  <footer className="port-footer"><span>{tutorial ? "操作教学 · 独立现场 · 不写入自主练习记录" : demonstration ? "课程内置示范 · 观察模式 · 不写入自主练习记录" : "教学情境 · 按批次安排，逐箱留痕 · 本地自动保存"}{r.busy ? " · 正在重放…" : ""}</span><button disabled={tutorial} onClick={onLegacy}>8 小时基础实训 / 旧版存档</button></footer>
  {!tutorial && !demonstration && offer && (v.mode === "practice" || manualOffer && !["running", "paused"].includes(v.status)) && <PortTutorialOffer unit={practiceUnit} onStart={unit => void enterTutorial(unit)} onSkip={() => { setOffer(false); setManualOffer(false); }}/>}
  {tutorial && r.tutorial && <PortTutorialOverlay tutorial={r.tutorial} root={root} scene={scene} status={v.status} feedback={r.error || (v.attempts.at(-1)?.outcome === "waiting" ? v.attempts.at(-1)!.message : "")} onReveal={revealTutorialStep} onContinue={() => void send({ kind: "resume" })} onExit={exitTutorial} onRestart={() => void retry()} onResources={() => { setTab(courseUnit === "arrival" || courseUnit === "departure" ? "ships" : "resources"); setStagePanel("operations"); }}/>}
 </section>;
}
...
function PlanEditor({ view: v, modeLocked, onApply, send, drop, courseUnit, readOnly = false }: {
  ...
  <fieldset disabled={!ready}><legend>开局配置</legend><fieldset disabled={!!courseUnit} className="port-scenario-fields"><div className="port-inline"><label>场次类型<select aria-label="场次类型" disabled={modeLocked} value={mode} onChange={e => setMode(e.target.value as PortMode)}><option value="practice">教学模式</option><option value="battle">实战模式</option></select></label><label>种子<input type="number" min={0} max={4294967295} value={c.seed} onChange={e => setC({ ...c, seed: Number(e.target.value) })}/></label></div><div className="port-inline"><label>平均到港间隔（分钟）<input type="number" min={241} max={1440} value={c.meanGapMinutes} onChange={e => setC({ ...c, meanGapMinutes: Number(e.target.value) })}/></label><label>到港波动<select value={c.variability} onChange={e => setC({ ...c, variability: e.target.value as PortConfig["variability"] })}><option value="low">低</option><option value="normal">中</option><option value="high">高</option></select></label></div><div className="port-inline"><label>大型船比例<input type="number" min={0} max={1} step={.05} value={c.largeShare} onChange={e => setC({ ...c, largeShare: Number(e.target.value) })}/></label><label>作业扰动<select value={c.disruption} onChange={e => setC({ ...c, disruption: e.target.value as PortConfig["disruption"] })}><option value="none">常态</option><option value="wind">大风窗口</option><option value="outage">岸桥故障</option></select></label></div>
   </fieldset><h3 data-tutorial-target="plan-equipment">设备选型 · 投入 {fmt(terminalBudget(p.equipment))} / 1,200</h3>{([["crane", "cranes"], ["vehicle", "vehicles"], ["yardMachine", "yardMachines"], ["gateSystem", "gates"]] as const).map(([model, count]) => <div className="port-inline" key={model}><label>设备型号<select aria-label={`${model}型号`} value={p.equipment[model]} onChange={e => setP({ ...p, equipment: { ...p.equipment, [model]: e.target.value } })}>{Object.entries(TERMINAL_EQUIPMENT[model]).map(([key, m]) => <option key={key} value={key}>{m.label} · {m.cost} 点</option>)}</select></label><label>数量<input aria-label={`${count}数量`} type="number" min={0} max={count === "vehicles" ? 16 : 6} value={p.equipment[count]} onChange={e => { const n = Number(e.target.value), equipment = { ...p.equipment, [count]: n }; if (count === "cranes") {
    ...
    } setP({ ...p, equipment }); }}/></label></div>)}<p>先确定设备和布局，开局后在“调度”中分配岗位与岸桥。大型船需要大伸距岸桥。</p><button data-tutorial-target="plan-apply" className="primary" onClick={() => void onApply(mode, c, p)}>应用开局方案</button></fieldset><small>固定道路与岸线；堆场位置决定运输距离。必须保留待核查空间。投入和运行费为教学点，不对应真实价格。</small></>;
}
