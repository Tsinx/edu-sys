...
export function PortTutorialOffer({ unit, onStart, onSkip }: { unit?: PortCourseUnit; onStart: (unit: PortCourseUnit) => void; onSkip: () => void }) {
  ...
  return <dialog ref={dialog} className="port-tutorial-offer" aria-labelledby="port-tutorial-question" onCancel={e => { e.preventDefault(); onSkip(); }}>
    ...
    <h2 id="port-tutorial-question">是否先学习本段操作？</h2><p>跟着高亮与手势，亲自点一次、拖一次。教学使用独立现场，原来的练习会留在原处。</p>
...
export function PortTutorialOverlay({ tutorial, root, scene, status, feedback, onReveal, onContinue, onExit, onRestart, onResources }: {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (!host) return;
      const next: Rect[] = [];
      const source=step?.sceneGesture&&width>900?scene.current?.project(step.sceneGesture.source):null;
  ...
  }, [stateKey, root, scene, replay]);
  const narrow = size.width <= 900, w = Math.min(narrow ? size.width - 24 : 338, size.width - 24), h = size.cardHeight;
  const candidates = [
  ...
  const gestures = { click: "点击高亮位置", context: "右键 / 操作按钮", input: "核对后填写", drag: "按住 → 拖动 → 松开", observe: "查看真实反馈" };
  return <div className="port-tutorial-layer" data-tutorial-step={key} data-tutorial-phase={step?.phase ?? "complete"}>
    <svg className="port-tutorial-spotlights" aria-hidden="true" viewBox={`0 0 ${size.width} ${size.height}`}>
      ...
      {!interacting && start && end && step?.gesture === "drag" && <path data-drag-trace={destination?.key.startsWith("scene:")?"stage":"panel"} d={`M ${start.x} ${start.y} L ${end.x} ${end.y}`} stroke="#f5b741" strokeWidth="3" strokeDasharray="7 6" fill="none"/>}
      {ghost && !interacting && step?.phase!=="running" && start && end && <g key={`${key}-${replay}-${start.x}-${start.y}-${end.x}-${end.y}`} className="port-tutorial-ghost" transform={`translate(${start.x} ${start.y})`}>
    ...
    <section ref={card} className="port-tutorial-coach" aria-label="操作教学引导" style={{ left: Math.max(12, position.x), top: position.y, width: w }}>
      ...
      {step ? <>{!compact && <><p>{step.repeated && !expanded && step.phase === "action" ? "同类操作：按高亮完成本批安排。需要帮助可展开详细提示。" : step.instruction}</p>{(!step.repeated || expanded) && <p className="port-tutorial-why">{step.why}</p>}{step.repeated && <button onClick={() => setExpanded(!expanded)}>{expanded ? "收起详细提示" : "展开详细提示"}</button>}</>}
