...
export function usePortOperations(storage: LabStorage | null | undefined, scope: string, initialMode: PortMode, locked: boolean, initialConfig = defaultPortConfig(), course?: { unit: PortCourseUnit; demo: boolean }) {
    ...
    const persist = useCallback(() => { if (original.current)
        return; try {
        storage?.setItem(portStorageKey(scope, modeRef.current), JSON.stringify(book.current));
        if (!locked)
    ...
    const readBook = useCallback((mode: PortMode) => {
        const raw = storage?.getItem(portStorageKey(scope, mode));
        original.current = null;
        if (!raw)
            return { active: "", final: null, entries: [] } as History;
        try {
            const h = JSON.parse(raw) as History;
            if (!Array.isArray(h.entries) || h.entries.some(e => typeof e.raw !== "string" || typeof e.id !== "string"))
    ...
    const launch = useCallback(async (mode: PortMode, config: PortConfig, plan: PortPlan, raw?: string, id?: string) => {
        setBusy(true);
        setError("");
        setBox(null);
        setLesson(null);
        ...
        if (!id)
            book.current.entries.push({ id: active.current, created: new Date().toISOString(), mode, seed: config.seed, status: "ready", second: 0, raw: "", score: null });
        const m = await post({ type: "init", mode, config, plan, raw, course });
        if (m.type === "error")
    ...
    useEffect(() => {
        ...
        w.onmessage = (event: MessageEvent<Message>) => {
            ...
            if (m.type === "state" && m.view && m.saved) {
                const v = m.view;
                ...
                if (e) {
                    Object.assign(e, { raw: m.saved, mode: v.mode, seed: v.config.seed, status: v.status === "running" && v.mode === "battle" ? "interrupted" : v.status, second: v.second, score: v.score.referenceReady && v.status === "completed" ? v.score.total : null });
                    persist();
                ...
                const configKey = JSON.stringify(v.config);
                if (!course && benchmarkConfig.current !== configKey) {
                    ...
                    b.onmessage = (event: MessageEvent<Message>) => { if (event.data.type === "benchmark" && benchmarkConfig.current === configKey) {
                        void post({ type: "reference", config: v.config, reference: event.data.reference });
                        b.terminate();
                    }
                    else if (event.data.type === "error")
                        setError(`参考成本计算失败：${event.data.message}`); };
                    b.postMessage({ id: 1, type: "benchmark", config: v.config });
                }
            }
        ...
        return () => { clearInterval(timer); document.removeEventListener("visibilitychange", visibility); persist(); w.terminate(); benchmark.current?.terminate(); worker.current = null; latest.current = null; benchmarkConfig.current = ""; pending.current.clear(); };
    }, [scope, storage, initialMode, course?.unit, course?.demo]); // Identity, module and demonstrations use isolated histories.
    ...
    const retry = async (newSeed = false, entry?: PortHistoryEntry) => {
        ...
        await launch(modeRef.current, config, plan);
    };
    const load = async (raw: string) => {
        try {
            const data = JSON.parse(raw);
            if (course) {
                if (data.schema !== "port-course/1.0" || data.unit !== course.unit || data.demo === true) throw new Error("请导入当前分段的自主练习记录；演示记录与综合场次不能作为本段练习。");
            ...
            if (data.schema !== "port-operations/3.0")
    ...
    return { view, lesson, demonstration, demoPlaying, setDemoPlaying, demoStep: () => { if (demoPlayingRef.current) setDemoPlaying(false); return post({ type: "demo-step" }); }, history, error, notice, speed, box, busy, send, configure, retry, load, selectFinal, openHistory, exportReport, inspect: (boxId: string) => post({ type: "inspect", boxId }), setSpeed: (n: number) => { if (latest.current?.mode === "practice") {
            tick();
            speedRef.current = n;
            setSpeedState(n);
            lastWall.current = Date.now();
