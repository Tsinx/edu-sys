...
当前 Linux 服务器通过 Caddy 和 systemd 用户服务运行，维护步骤见 [Linux 部署手册](deploy/linux/README.md)。2026-09-15 已完成学生名单导入、港口管理概论选课限制和学生 AI 停用；管理员保留教师管理权限。账号、名单、密码和密钥保存在被 Git 忽略的本机目录中，克隆仓库不会带入这些数据。DashScope 密钥已从本机终端配置导入服务配置，管理员的课下数字人和课堂助手均已通过真实云端文字回复验证，学生 AI 保持停用。
