...
- 账号可绑定选课范围，服务器会过滤课程和课堂列表并限制直接访问。账号创建工具只创建账号，不会自动绑定课程；没有限制记录的旧账号保留原权限。
...
- 服务端支持 `dashscope_api_key` 小写环境变量别名。设置 `EDU_STUDENT_AI_ENABLED=false` 并重启后端，可停用学生的 AI 问答、语音识别和语音合成，保留课件阅读与仿真；教师权限不受此开关影响。账号密码默认至少12位，`EDU_ACCOUNT_MIN_PASSWORD_LENGTH` 可显式调整，最低6位。
