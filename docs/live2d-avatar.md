...
继续使用已有的检测输入、按键输入和文字输入。切换形象会中断当前讲解。Live2D 无需启动 OpenAvatarChat；课堂录音统一通过平台的实时语音连接，文字问答与自学助手使用各自配置的服务。停止/打断会清空音频并关闭口型。收起数字人继续保留现有语音会话。
...
- `campus/BrowserAvatarSurface.tsx`：播放课堂实时语音流，保留文字回答和预设讲解的 TTS 与字幕链路；取消旧请求时隔离迟到回调。只有选择 LAM 才走 LAM 分支与状态轮询；LAM 暂不支持课堂实时录音。
...
角色采用音量口型，从实际播放的音频信号驱动嘴部。课堂语音直接播放实时模型返回的 PCM 分片；文字回答仍在收到完整文本后发起 TTS 并排队播放。
